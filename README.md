...
**배포:** https://blog-agent-lnpt.onrender.com
...
1. **카카오 로그인** — 카카오 계정으로 소셜 로그인
2. **말투 프로파일 생성** — 본인이 쓴 블로그 글 샘플을 붙여넣으면 AI가 말투를 분석해 프로파일 저장
3. **블로그 글 생성** — 가게 이름 검색 → 사진 업로드 → 저장된 말투 프로파일 선택 → 글 생성
...
`.env` 파일을 먼저 생성하고 아래 환경변수를 설정해야 합니다.
